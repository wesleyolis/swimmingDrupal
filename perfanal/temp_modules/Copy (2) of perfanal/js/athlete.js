...
//long course 0
	...
	parent.document.getElementById('s'+prev).innerHTML ="";
		...
		if(multi==1 || multi==3)
		body+="<td width='80px' "+((d1==1 || multi==1)?'style="font-weight: bold"':'')+" nowrap >SC&nbsp;Faster</td><td width='80px' "+((d1==1 || multi==1)?'style="font-weight: bold"':'')+" nowrap>SC&nbsp;Slower</td>";
		if(multi==0  || multi==3)
		body+="<td width='80px'"+((d1==0 || multi==0)?'style="font-weight: bold"':'')+" nowrap>LC&nbsp;Faster</td><td width='80px' "+((d1==0 || multi==0)?'style="font-weight: bold"':'')+"  nowrap>LC&nbsp;Slower</td><tr>";
		
		body+="<tr  class='white'><td class='white'>"+Time(d0)+"&nbsp;";
		
		if(multi==1 || multi==3)
		{
		body+="</td><td class='"+((d1==1 || multi==1)?(((d0<=d2 & d2!=0 & d0!=0) || (d5==0 & d4==0 & d2==0))?'green':'red'):'white')+"'>"+Time(d2);
		body+="</td><td class='"+((d1==1 || multi==1)?(((d0>=d3 & d3!=0 & d0!=0) || (d5==0 & d4==0 & d3==0))?'green':'red'):'white')+"'>"+Time(d3);
		}
		if(multi==0  || multi==3)
		{
		body+="</td><td class='"+((d1==0 || multi==0)?(((d0<=d4 & d4!=0 & d0!=0) || (d4==0 & d3==0 & d2==0))?'green':'red'):'white')+"'>"+Time(d4);
		body+="</td><td class='"+((d1==0 || multi==0)?(((d0>=d5 & d5!=0 & d0!=0) || (d5==0 & d3==0 & d2==0))?'green':'red'):'white')+"'>"+Time(d5);
		}
		body+="</td><tr>";
		body+="</table>";
