<?php
 ...
 $headers[] = array('data' => t('#'), 'width' => '20px');
   ...
   $query.=" (".$tm4db.".result_".$season." as r left join ".$tm4db.".athlete_".$season." as a on (r.ATHLETE= a.Athlete)) left join ".$tm4db.".team_".$season." as t on (a.Team1=t.Team)";
   $query.=" Where a.Group='A' and r.points>0 and".$Where." and r.I_R='I' GROUP BY  r.ATHLETE having Sum(r.POINTS)>0 order by Sum_Points DESC,a.last";
   //$output.=$query;
