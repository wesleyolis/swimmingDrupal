<?php
 $query = "select SQL_CACHE  distinct m.QTSS,m.QTS,m.QTLS,m.QTL, m.MName FROM ".$tm4db.".meet_".$season." as m  where m.Meet=%d";
			 ...
			 $headers[] = array('data' => t('SC Faster'), 'width' => '90px');
		       ...
		       $result = db_query("Select FastCut,SlowCut,Fast_L,Slow_L, Round(Fee,2) as Fee,Session, MtEvent, IF(Count(MtEvX)=1,Lo_Hi,'*') as Lo_Hi ,MtEv, IF(Count(Distinct Sex)=1,Sex,'X') As Sex, Distance, Stroke, I_R,If(Count(*)>0,Count(*),1) as Events, Course,Division FROM ".$tm4db.".mtevente_".$season." as e WHERE e.MtEv>0 and  Meet=%d ".(arg(4)!=null?' and e.Lo_Hi='.arg(4):'')." Group by MtEv order by Session, MtEv",arg(3));
