<?php
 $query = "select SQL_CACHE  distinct m.QTSS,m.QTS,m.QTLS,m.QTL,IF(e.Meet Is Null,1,0) as results, m.MName FROM ".$tm4db.".meet_".$season." as m left join ".$tm4db.".mtevent_".$season." e on (m.Meet=e.Meet) where m.Meet=%d";
		       ...
		       If($display_link==0)
			 {
		       ...
		       $result = db_query("Select DISTINCT MtEvent, IF(Count(MtEvX)=1,Lo_Hi,'*') as Lo_Hi ,MtEv, IF(Count(Distinct Sex)=1,Sex,'X') As Sex, Distance, Stroke, I_R,If(Count(*)>0,Count(*),1) as Events, Course,Division, Sum(Results) as Results FROM ".$tm4db.".mtevent_".$season." as e WHERE Meet=%d ".(arg(4)!=null?' and e.Lo_Hi='.arg(4):'')." Group by MtEv order by MtEv",arg(3));
