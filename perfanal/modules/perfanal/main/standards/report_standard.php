<?php
   ...
   if(arg(2)==null)
	...
	while ($object = db_fetch_object($result))
	  {
	     $link = 'report_standard/'.arg(1).'/'.$object->Meet;
	...
	if(arg(3)==null)
	  {
	     ...
	     while($object = db_fetch_object($results))
	       $rows[] = array(l($object->StdFile,'report_standard/'.arg(1).'/'.arg(2).'/'.$object->StdFile),$object->Descript,$object->Year);
	...
	else
	  {
	     ...
	     $result = db_query("Select * From ".$tm4db.".stdname_".$season." Where StdFile='%s' ",arg(3));
	     ...
	     if(db_num_rows($result)>0)
	       {
		  $result = db_query("select m.*,IF(m.AgeUp=null,m.Start,m.AgeUp) as AgeUp from ".$tm4db.".meet_".$season." as m Where m.meet=%d",arg(2));
		  $object = db_fetch_object($result);

		  drupal_set_title($object->MName.", ".arg(3)." - National Gala Standards Statistics");
