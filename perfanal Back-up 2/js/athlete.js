...
function dis_splits(id,s)
{
	...
	if(s!=null & s !='' )
	{
		...
		body="<table width='"+width+"px' border='2' cellpadding='0' bgcolor='#FFFFFF' bordercolor='#000000' style='border-collapse: collapse'>";
...
function dis_qt(id,s)
{
	...
	if(s!=null & s !='' )
	{
		...
		body="<table width='420px' border='2' cellpadding='0' bgcolor='#FFFFFF' bordercolor='#000000' style='border-collapse: collapse'>";
		...
		body+="<tr><td>"+Time(data[0])+"</td><td class='"+((data[1]=='S')?((data[0]<=data[2])?'green':'red'):'')+"'>"+Time(data[2])
		+"</td><td class='"+((data[1]=='S')?((data[0]>=data[3])?'green':'red'):'')+"'>"+Time(data[3])
		+"</td><td class='"+((data[1]=='L')?((data[0]<=data[4])?'green':'red'):'')+"'>"+Time(data[4])
		+"</td><td class='"+((data[1]=='L')?((data[0]>=data[5])?'green':'red'):'')+"'>"+Time(data[5])+"</td><tr>";
		body+="</table>";
		parent.document.getElementById('s'+id).style.top="5px";
		parent.document.getElementById('s'+id).innerHTML =body;
