	...
	$rankdate = ranking_date($season);
	     ...
	     */
	     ...
	     if($ranking_stage <5)
		   ...
		   db_query("set @".$grp."='-1'");
		  $query=" update ".$tm4db."result_".$season." as r";
		  $query.=" set r.RAll = if((@".$grp." != (@".$grp.":=concat(r.athlete,r.COURSE,r.STROKE,r.distance,r.NT,r.I_R)) and r.NT=0 and r.I_R = 'I'),1,if(r.points!=0,2,0))";
		  $query.=" Where r.RAll>=0 and r.NT=0";
		  $query.=" order by r.Athlete,r.Course,r.Stroke,r.Distance,r.score;";
		 //$output.=$query;
		// echo $query;
	    ...
	    // if(!($meet_type =='ALL' & $meet_type =='LSC' & $meet_type_fina =='ALL' & $meet_type_fina =='LSC'))//$meet_type !='' 
	     if(!($meet_type =='ALL' & $meet_type_fina =='ALL'))//$meet_type !='' 
		  ...
		  if($ranking_stage <6)
			...
			if($i==1)
			{
			$query_rev.="r.Rtype=if(r.RAll<0,-1,c.tindex)";
			}
			else
			{
		 ...
		 while($object = db_fetch_object($results))
		  {$grp='grp'.rand(0,500);
			...
			//  db_query('INSERT INTO TargetTable (result,sub) ');
		        db_query("set @".$grp."='0'");
		       $query=" update ".$tm4db."result_".$season." as r";
		       $query.=" set r.RType = if(r.RType!=".$object->tindex.",r.RType,if((@".$grp." != (@".$grp.":=concat(r.athlete,r.COURSE,r.STROKE,r.distance,r.NT,r.I_R)) and r.NT=0 and r.I_R = 'I'),".$object->tindex.",if(r.POINTS!=0,".($object->tindex+1).",0)))";
		      
		       $col_Rtype='';
		       for($i=2;$i<=$max_sanctions;$i++)
		       {
			       $query.=", r.RType".$i." = if(r.RType".$i."!=".$object->tindex.",r.RType".$i.",if((@".$grp." != (@".$grp.":=concat(r.athlete,r.COURSE,r.STROKE,r.distance,r.NT,r.I_R)) and r.NT=0 and r.I_R = 'I'),".$object->tindex.",if(r.POINTS!=0,".($object->tindex+1).",0)))";
		      
			       $col_Rtype.= " or r.RType".$i."=".$object->tindex;
		       }
		       
		       $query.=" Where  r.NT=0 and ( r.RType=".$object->tindex." ".$col_Rtype.")";
		       $query.=" order by r.Athlete,r.Course,r.Stroke,r.Distance,r.score;";
		      //$output.=$query;
