  ...
  $tm4db = variable_get('perfanal_database', 'perfanal');
   ...
   if(arg(1)==null)
	...
	$output='';
		  ...
		  db_query("Alter Table ".$tm4db."code_".$season."  change `DESC` `_desc` varchar(30) ");
		  ...
		  db_query("Alter Table ".$tm4db."code_".$season." ADD KEY `ABBR` (`ABBR`,`TYPE`) ");
		  ...
		  $result = db_query($query);
		     //Section must be exported to optimization so only happens once.
		     	db_query("DROP TABLE IF EXISTS ".$tm4db."meet_sanctions_".$season);
			  db_query("CREATE TABLE IF NOT EXISTS ".$tm4db."meet_sanctions_".$season." (meet INT, abbr VARCHAR(5),c smallint,include smallint) ENGINE=MYISAM;");
			  db_query("Alter Table ".$tm4db."meet_sanctions_".$season." ADD KEY `ABBR` (`ABBR`) ");
			  db_query("Alter Table ".$tm4db."meet_sanctions_".$season." ADD KEY `meet` (`meet`) ");
			 ...
			 while($object = db_fetch_object($results))
			 {
				 ...
				 for($i=0;$i<(sizeof($keys)-1);$i++)
				 {
					 $pos=strpos($keys[$i],'!');
					 if($pos==false)
					 db_query("INSERT delayed into ".$tm4db."meet_sanctions_".$season." (meet,abbr,c,include) values(".$object->meet.",'".substr($keys[$i],0,strlen($keys[$i]))."',".($i+1).",1)");
					 else
					 db_query("INSERT delayed into ".$tm4db."meet_sanctions_".$season." (meet,abbr,c,include) values(".$object->meet.",'".substr($keys[$i],0,$pos)."',".($i+1).",0)");
				 }
		  ...
		  foreach($commands as $table =>$command)
		    {
			 ...
			 //db_query("Alter Table ".$tm4db."result_".$season." ADD KEY `RALL` (`RFINA`) ");
			 db_query("Alter Table ".$tm4db."athlete_".$season." ADD FINA_age tinyint ");
	...
	*/
		  ...
		  db_query("Alter Table ".$tm4db."athlete_".$season." add  KEY `LastName` (`Last`), add  KEY `First` (`First`),add  KEY `Initial` (`Initial`)");
...
//		  db_query("Alter Table ".$tm4db."meet_".$season." ADD UNIQUE KEY `ZID` (`Meet`),add  KEY `MName` (`MName`,`Start`),add KEY `Start` (`Start`)");
		...
		db_query("Alter Table ".$tm4db."meet_".$season." add KEY `Start` (`Start`)");
		  ...
		  db_query("Alter Table ".$tm4db."mtevent_".$season." ADD  UNIQUE KEY `ZID` (`MtEvent`)");
		  db_query("Alter Table ".$tm4db."mtevent_".$season." add KEY `Meet` (`Meet`,`MtEv`,`MtEvX`)");
		    db_query("Alter Table ".$tm4db."mtevent_".$season." add KEY `MLo_Hi` (`Meet`,`Lo_Hi`)");
		    

		  $output.= "Formating table: mtevente <br>";
		  db_query("Alter Table ".$tm4db."mtevente_".$season." ADD UNIQUE KEY `ZID` (`MtEvent`) ");
		  db_query("Alter Table ".$tm4db."mtevente_".$season." add  KEY `Meet2` (`Meet`,`MtEv`,`MtEvX`) ");
		...
		db_query("Alter Table ".$tm4db."team_".$season." ADD  KEY `tcntry_lsc` (`TCntry`,`lsc`)");
		  ...
		  db_query("Alter Table ".$tm4db."result_".$season." ADD UNIQUE KEY `ZID` (`RESULT`)");
		 db_query("Alter Table ".$tm4db."result_".$season." add  KEY `MEET` (`MEET`,`MTEVENT`,`I_R`,`F_P`) ");
		 db_query("Alter Table ".$tm4db."result_".$season."  add KEY `Fast` (`ATHLETE`,`COURSE`,`STROKE`,`DISTANCE`) ");
...
//		  db_query("Alter Table ".$tm4db."team_".$season." ADD UNIQUE KEY `ZID` (`Team`), add KEY `LSC` (`LSC`,`TCode`)");
		  ...
		  $pref_temp_yes=true;
		  $result=null;
		  $result = db_query("Select distinct  pref_temp from ".$tm4db."athlete_".$season."");
		  ...
		  if($pref_temp_yes)
		  {
			  db_query("Alter Table ".$tm4db."athlete_".$season." ADD pref_temp varchar(20)");
			  ...
			  $query = "UPDATE ".$tm4db."athlete_".$season." SET Pref = TRIM(CONCAT(UCase(LEFT(Pref,1)),LCase(SUBSTRING(Pref,2))))";
	
			  $result = db_query($query);
			  $output.= "Formating Pref Name<br>";
			   
			  $query = "UPDATE ".$tm4db."athlete_".$season." as a SET a.pref_temp = a.Pref, a.Pref = a.First";
			  $result = db_query($query);
			  $query = "UPDATE ".$tm4db."athlete_".$season." SET First = if(pref_temp='',First,pref_temp)";
			  $result = db_query($query);
		  ...
		  db_query("update ".$tm4db."result_".$season." as r set r.RBest = if(((@".$grp." != (@".$grp.":=concat(r.athlete,r.COURSE,r.STROKE,r.distance,r.NT,r.I_R))) and r.NT=0 and r.I_R != 'R'),1,0) order by r.Athlete,r.Course,r.Stroke,r.Distance,r.score");
	...
	$query.="Where f.years=2004 and r.I_R!='R' and r.NT =0 and a.sex='M';";*/
		  ...
		  $query="update ".$tm4db."splits_".$season." as s inner join (select distinct r.distance/(s.splitindex*25) as updatefactor, r.result ";
		  $query.=" from ".$tm4db."result_".$season." as r inner join ".$tm4db."splits_".$season." as s on (r.result = s.splitID) where r.I_R!='R' and r.nt=0 and r.score!=0 and r.score = s.split and ((s.splitindex*25)/r.distance=0.5 or (s.splitindex*25)/r.distance=2) ) as r2 on (r2.result = s.splitID) set s.splitindex = floor(s.splitindex*r2.updatefactor)";
	...
	*/
	  ...
	  $output.="<br>".l("Click Here update rankings and to finsh the setup.",'updaterankings/'.arg(1).'/manual');
     }
?>